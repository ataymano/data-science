...
class Processor:
    ...
    default_slot_processor = lambda i, o: {
        '_inc': o.get('_inc', [])}
    
    default_outcome_processor = lambda i, o: {
    ...
    def _process_decision(self, line):
        ...
        for i, o in enumerate(parsed['c']['_multi']):
            actions[i] = dict(ChainMap(*[p(i, o) for p in self.processors['_multi']]))

        slots = [None] * len(parsed['c']['_slots'])
        for i, o in enumerate(parsed['c']['_slots']):
            slots[i] = dict(ChainMap(*[p(i, o) for p in self.processors['_slots']]))
        
        outcomes = [None] * len(parsed['_outcomes'])
        for i, o in enumerate(parsed['_outcomes']):
            outcomes[i] = dict(ChainMap(*[p(i, o) for p in self.processors['_outcomes']]))
    ...
    def _process_decision_csv(self, line):
        ...
        if self.processors['_multi']:
            for i, o in enumerate(parsed['c']['_multi']):
                actions = dict(ChainMap(*[p(i, o) for p in self.processors['_multi']]))
            ...
            for i, o in enumerate(parsed['_outcomes']):
                outcomes = dict(ChainMap(*[p(i, o) for p in self.processors['_outcomes']]))
