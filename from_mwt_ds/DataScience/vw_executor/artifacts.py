...
def _audit_to_dict(first, second):
    prediction, tag = (* first.strip().split(' ') + [None], )[:2]
    result = {}
    result[('prediction', '')] = float(prediction) 
    result[('tag', '')] = tag
    for f in second.strip().split('\t'):
        feature_info, ssgrad = (* f.split('@') + [None], )[:2]
        name, hash, value, weight = (* feature_info.split(':'), )
        result[(name, 'hash')] = int(hash)
        result[(name, 'value')] = float(value)
        result[(name, 'weight')] = float(weight)
        result[(name, 'ssgrad')] = float(ssgrad) if ssgrad else None
    return result
...
class Audit(Artifact):
    ...
    def _process(self):
        ...
        it = iter(self.raw)
        self._table = pd.DataFrame([_audit_to_dict(x, next(it)) for x in it])
        self._table.columns = pd.MultiIndex.from_tuples(self._table.columns)
