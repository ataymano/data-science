...
from vw_executor.artifacts import Output, Audit
...
class Task:
    ...
    def _prepare_args(self, cache):
        ...
        self.audit_path = cache.path.joinpath(cache.get_path(opts, '-audit', salt, self._logger))
...
class Job:
    ...
    @property
    def audit(self):
        return pd.concat([t.audit.table.assign(file=i)
                          for i, t in enumerate(self._tasks)]).reset_index().set_index(['file', 'index'])
